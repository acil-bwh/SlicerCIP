...
///
/// Airway nodes contain trajectories ("fibers") from tractography, internally represented as vtkPolyData.
...
/// Visualization parameters for these nodes are controlled by the vtkMRMLAirwayDisplayNode class.
//
...
class vtkMRMLAirwayDisplayNode;
...
class VTK_SLICER_TRACTOGRAPHYDISPLAY_MODULE_MRML_EXPORT vtkMRMLAirwayNode : public vtkMRMLModelNode
{
public:
  static vtkMRMLAirwayNode *New();
  vtkTypeMacro(vtkMRMLAirwayNode,vtkMRMLModelNode);
  //vtkTypeMacro(vtkMRMLAirwayNode,vtkMRMLTransformableNode);
