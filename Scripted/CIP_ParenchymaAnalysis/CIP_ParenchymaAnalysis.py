...
class CIP_ParenchymaAnalysisWidget(ScriptedLoadableModuleWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.rTags = (
            "WholeLung", "RightLung", "LeftLung", "RUL", "RML", "RLL", "LUL", "LLL", "LUT", "LMT", "LLT", "RUT", "RMT",
            "RLT")
        if not parent:
        ...
        if self.CTNode:
            for color in ['Red', 'Yellow', 'Green']:
                slicer.app.layoutManager().sliceWidget(
                    color).sliceLogic().GetSliceCompositeNode().SetBackgroundVolumeID(self.CTNode.GetID())
        else:
            for color in ['Red', 'Yellow', 'Green']:
                slicer.app.layoutManager().sliceWidget(
                    color).sliceLogic().GetSliceCompositeNode().SetBackgroundVolumeID('None')
    ...
    def createLungLabelMap(self):
        ...
        input_array = input_array.reshape(shape[2], shape[1],
