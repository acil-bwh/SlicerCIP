...
class StructuresParameters(object):
    INF = 100000
    maxRegionTypeId = 0     # Max code (following ChestImagingPlatform conventions)
    maxStructureCode = 0    # Max code (following bounding box customized structure codes)
    """
        Ids:
        - StructureId
        - ChestRegion
    ...
    def getItemFromStructureIntId(self, id):
        for k,v in self.structureTypes.iteritems():
            if v[0]==id:
                return k
        return None
    ...
    def getDescriptionItemWithoutPlane(self, structureId):
        """Return the description label without the plane (Ex: 'Transversal Aorta' instead of 'Transversal Aorta (Axial)'"""
        item = self.getItem(structureId)
        descr = item[self.DESCRIPTION]
        plane = item[self.PLANE]
        if plane == 'A':
            return descr.replace(" (Axial)", "")
        if plane == 'S':
            return descr.replace(" (Sagittal)", "")
        if plane == 'C':
            return descr.replace(" (Coronal)", "")
    ...
    def readStructuresFromFile(self, xmlFilePath, colorPriorityForRegions=True):
        ...
        for structure in root.findall("Structure"):
            ...
            # Save the maximum codes for future use of colormap nodes
            self.maxStructureCode = max(self.maxStructureCode, l[self.STRUCTURE_ID])
            self.maxRegionTypeId = max(self.maxRegionTypeId,
                            Util.get_value_from_chest_type_and_region(l[self.CHEST_TYPE_ID], l[self.CHEST_REGION_ID]))
    ...
    def createColormapNode(self, colormapName):
        """ Create a colormap node with one color per structure.
        When regularLabelmap==True, two structures will have the same color even if they are in different planes (ex:
        "Whole Heart (Axial)" and "Whole Heart (Saggital)" will both match to the region "Heart").
        Otherwise, every structure will have a different color
        :param colormapName: the colormap node will be named like this
        :param regularLabelmap: indicates if we are working in "bounding box mode" or "regular labelmap mode"
        :return:
        """
        # if regularLabelmap:
        #     # Full labelmap mode
        #     colorNode = SlicerUtil.createNewColormapNode(colormapName, numberOfColors=self.maxRegionTypeId + 1)
        #     for structureId, structure in self.structureTypes.iteritems():
        #         colorNode.SetColor(Util.get_value_from_chest_type_and_region(structure[self.CHEST_TYPE_ID], structure[self.CHEST_REGION_ID]),
        #                            self.getDescriptionItemWithoutPlane(structureId),
        #                            structure[self.RED], structure[self.GREEN], structure[self.BLUE])
        # else:
        # Bounding box mode
        colorNode = SlicerUtil.createNewColormapNode(colormapName, numberOfColors=self.maxStructureCode + 1)
        for structureId, structure in self.structureTypes.iteritems():
            colorNode.SetColor(structure[self.STRUCTURE_ID], structure[self.DESCRIPTION],
                               structure[self.RED], structure[self.GREEN], structure[self.BLUE])
        # Background shouldn't have any opacity
